...
boolean showFPFlag = ("true".equals(ParamUtil.getString(request,"showFPPopup"))? true: false);
System.out.println("showFPFlag =============== " + showFPFlag);
%>

<portlet:actionURL var="forgotPasswordURL">
	<portlet:param name="struts_action" value="/login/forgot_password" />
	<portlet:param name="showFPPopup" value='<%= String.valueOf(showFPFlag) %>' />
</portlet:actionURL>
		...
		<c:choose>
			<c:when test="<%= user2 == null %>">
				...
				<div class="np-input-button">
					<aui:button type="submit" value='<%= PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "send-password-reset-link" : "send-password-reset-link" %>' cssClass="np-submit"  />
					<br><br>
					<a href="/web/guest/home?p_p_id=58&p_p_state=normal&_58_showLoginPopup=true">Back to Login Form</a>
				</div>
...
<aui:script use="aui-base">
			$(document).ready(function() {
				<c:if test='<%= showFPFlag %>'>
	 				$.fancybox.open('#loginBox');
	 				$(".un-login-box02").hide();
				</c:if>
				$(".taglib-captcha").each(function(){
					$("#form-multipart").css("display","none");
				});
			});
</aui:script>
<style>
	.aui label{ display: block !important; }
	.un-login-title01{ text-align: left; }
	.np-desktop-loginfix{ margin-bottom: 50px; }
	.forget-signIn { padding-left:100px; }
</style>
