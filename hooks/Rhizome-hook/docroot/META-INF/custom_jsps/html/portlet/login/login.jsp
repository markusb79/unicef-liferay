...
<c:choose>
	...
	<c:otherwise>
		...
		boolean showLoginFlag = ("true".equals(ParamUtil.getString(request,"showLoginPopup"))? true: false);
		...
		<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="fm">
			...
			<c:choose>
				...
				<c:when test='<%= ParamUtil.getString(request, "status").equals("success")%>'>
				`	<div class="alert alert-success">
						We have successfully sent new password to your registered email address. Please check your email to continue.
					</div>
				</c:when>
			</c:choose>

			<liferay-ui:error exception="<%= AuthException.class %>" message="authentication-wrong-password-entered" />
			...
			<liferay-ui:error exception="<%= UserPasswordException.class %>" message="authentication-wrong-password-entered" />
			...
			<aui:fieldset>
				...
				%>
					...
					<div class="un-input-s1"> 
						<aui:input autoFocus="<%= windowState.equals(LiferayWindowState.EXCLUSIVE) || windowState.equals(WindowState.MAXIMIZED) %>" cssClass="un-input-s1 np-select-desktopfix" placeholder="Email Address" name="login" showRequiredLabel="<%= false %>" type="text" label="">
							<aui:validator name="required" />
						</aui:input>
					</div>
	               <!--  <div class="np-input"> -->
	               <div class="un-input-s1">
						<aui:input  name="password" cssClass="np-form-control" showRequiredLabel="<%= false %>" type="password" label="" placeholder="Password" value="<%= password %>">
							<aui:validator name="required" />
						</aui:input>
	                </div> 
					...
					<div class="clearfix">	
						...
						<aui:button type="submit" value="Sign In" cssClass="un-submit-link" />
							
							<!-- Rememeber me check box comment Not required -->
							<%-- <div class="clearfix">
								<c:if test="<%= company.isAutoLogin() && !PropsValues.SESSION_DISABLED %>">
									<aui:input checked="<%= rememberMe %>" name="rememberMe" type="checkbox" cssClass="un-forgot-link-v2" />
								</c:if>
							</div> --%>
