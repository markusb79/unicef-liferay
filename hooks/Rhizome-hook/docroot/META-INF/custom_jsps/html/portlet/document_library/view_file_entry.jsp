...
<%
	...
	String referringPortletResource = ParamUtil.getString(request,
			"referringPortletResource");

	String uploadProgressId = "dlFileEntryUploadProgress";

	FileEntry fileEntry = (FileEntry) request
			.getAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRY);
	...
	FileVersion fileVersion = (FileVersion) request
			.getAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_VERSION);
	...
	if (fileVersion != null) {
		versionSpecific = true;
			...
			|| DLFileEntryPermission.contains(permissionChecker,
					fileEntry, ActionKeys.UPDATE)) {
	...
	if (fileVersion.getModel() instanceof DLFileVersion) {
		DLFileVersion dlFileVersion = (DLFileVersion) fileVersion
				.getModel();
	...
	if (PropsValues.DL_FILE_ENTRY_CONVERSIONS_ENABLED
		...
		conversions = (String[]) DocumentConversionUtil
				.getConversions(fileVersion.getExtension());
	}
	...
	if (portletDisplay.isWebDAVEnabled()) {
		webDavUrl = DLUtil
				.getWebDavURL(themeDisplay, folder, fileEntry);
	}
	...
	AssetEntry layoutAssetEntry = AssetEntryLocalServiceUtil
			.fetchEntry(DLFileEntryConstants.getClassName(),
					assetClassPK);
...
<c:if test="<%=showHeader && (folder != null)%>">
	...
	<!-- Added by Abhishek - Following div for the gray header -->
	<div class="sg-header clearfix buffer-bottom np-repos-headfix01">
		<liferay-ui:header backURL="<%=redirect%>"
			localizeTitle="<%=false%>" title="<%=fileVersion.getTitle()%>" />
	</div>
</c:if>
...
<div class="view">
	<c:if test="<%=showActions%>">
		<liferay-ui:app-view-toolbar>
			<aui:button-row cssClass="edit-toolbar"
	...
	<aui:row>
		<aui:col cssClass="lfr-asset-column-details" width="<%=70%>">
			...
			<div class="alert alert-error hide"
				id="<portlet:namespace />openMSOfficeError"></div>

			<c:if
				test="<%=(fileEntry.getLock() != null)
				...
				<c:choose>
						...
						<div class="alert alert-success">
							<c:choose>
								<c:when test="<%=lock.isNeverExpires()%>">
									<liferay-ui:message
										key="you-now-have-an-indefinite-lock-on-this-document" />
								</c:when>
								<c:otherwise>

									<%
										String lockExpirationTime = StringUtil
																			.toLowerCase(LanguageUtil
																					.getTimeDescription(
																							pageContext,
									...
									<%=LanguageUtil
												.format(pageContext,
														"you-now-have-a-lock-on-this-document",
														lockExpirationTime,
														false)%>
								</c:otherwise>
							</c:choose>
					...
					<c:otherwise>
						<div class="alert alert-error">
							<%=LanguageUtil
										.format(pageContext,
												"you-cannot-modify-this-document-because-it-was-locked-by-x-on-x",
												new Object[]
												{
														HtmlUtil.escape(PortalUtil.getUserName(
																...
																String.valueOf(lock
																		.getUserId()))),
														dateFormatDateTime.format(lock
																.getCreateDate()) },
												false)%>
						</div>
					</c:otherwise>
			...
			<div class="body-row">
			<div class="np-repos-head2-fix01">
				<div class="document-info" style="">
...
<div style=" width:100%; display:table;">
 	...
 	String displayURL = StringPool.BLANK;
 				...
 				User userDisplay = UserLocalServiceUtil
 						.fetchUser(fileEntry.getUserId());

 				if (userDisplay != null) {
 					displayURL = userDisplay
 							.getDisplayURL(themeDisplay);
 				}
 %> 
 
 
 <liferay-ui:icon image="" label="<%=true%>"
								...
								new Object[]
								{
										...
										dateFormatDateTime.format(fileEntry
												.getCreateDate()) })%>' />
...
<div id="part3">
						<c:if test="<%=enableRatings
									&& fileEntry.isSupportsSocial()%>">
							<span class="lfr-asset-ratings"> <liferay-ui:ratings
					...
					<c:if
						...
						<div class="lfr-asset-tags">
							<liferay-ui:asset-tags-summary
								className="<%=DLFileEntryConstants.getClassName()%>"
								classPK="<%=assetClassPK%>" message="tags" />
						</div>
					</c:if>
				</div>
				</div>

				<aui:model-context bean="<%=fileVersion%>"
					...
					<div>
						...
						<%
							int previewFileCount = 0;
										...
										if (hasAudio) {
											...
											previewFileURL = DLUtil.getPreviewURL(fileEntry,
													fileVersion, themeDisplay,
													previewQueryString);
										} else if (hasVideo) {
											previewQueryString = "&videoPreview=1";

											videoThumbnailURL = DLUtil.getPreviewURL(fileEntry,
													fileVersion, themeDisplay,
													"&videoThumbnail=1");
										}
										...
										if (Validator.isNotNull(previewQueryString)) {
											if (hasAudio) {
												...
												for (int i = 0; i < PropsValues.DL_FILE_ENTRY_PREVIEW_AUDIO_CONTAINERS.length; i++) {
													previewFileURLs[i] = DLUtil
															.getPreviewURL(
																	...
																	previewQueryString
																			+ "&type="
																			+ PropsValues.DL_FILE_ENTRY_PREVIEW_AUDIO_CONTAINERS[i]);
												}
											} else if (hasVideo) {
												...
												previewFileURLs[0] = DLUtil.getPreviewURL(
										...
										request.setAttribute(
												"view_file_entry.jsp-supportedAudio",
												String.valueOf(hasAudio));
										request.setAttribute(
												"view_file_entry.jsp-supportedVideo",
												String.valueOf(hasVideo));

										request.setAttribute(
												"view_file_entry.jsp-previewFileURLs",
												previewFileURLs);
										request.setAttribute(
												"view_file_entry.jsp-videoThumbnailURL",
												videoThumbnailURL);
						%>
						...
						<c:choose>
							<c:when test="<%=previewFileCount == 0%>">
								<c:choose>
									<c:when
										test="<%=!DLProcessorRegistryUtil
												.isPreviewableSize(fileVersion)
												&& (AudioProcessorUtil
														.isAudioSupported(fileVersion
																.getMimeType())
														|| ImageProcessorUtil
																.isImageSupported(fileVersion
																		.getMimeType())
														|| PDFProcessorUtil
																.isDocumentSupported(fileVersion
																		.getMimeType()) || VideoProcessorUtil
															.isVideoSupported(fileVersion
																	.getMimeType()))%>">
										<div class="alert alert-info">
											<liferay-ui:message
												key="file-is-too-large-for-preview-or-thumbnail-generation" />
										</div>
									</c:when>
									<c:when
										test="<%=AudioProcessorUtil
												.isAudioSupported(fileVersion)
												|| ImageProcessorUtil
														.isImageSupported(fileVersion)
												|| PDFProcessorUtil
										...
										<div class="alert alert-info">
											<liferay-ui:message
												key="generating-preview-will-take-a-few-minutes" />
										</div>
									</c:when>
								</c:choose>
							</c:when>
							<c:otherwise>
								<c:choose>
									<c:when test="<%=hasAudio%>">
										<div class="lfr-preview-audio"
											id="<portlet:namespace />previewFile">
											<div class="lfr-preview-audio-content"
												id="<portlet:namespace />previewFileContent"></div>
										</div>

										<liferay-util:include
											page="/html/portlet/document_library/player.jsp" />
									</c:when>
									<c:when test="<%=hasImages%>">
										<div class="lfr-preview-file lfr-preview-image"
											id="<portlet:namespace />previewFile">
											<div
												class="lfr-preview-file-content lfr-preview-image-content"
									...
									<c:when test="<%=hasVideo%>">
										<div class="lfr-preview-file lfr-preview-video"
											id="<portlet:namespace />previewFile">
											<div
												class="lfr-preview-file-content lfr-preview-video-content">
										...
										<div class="lfr-preview-file"
											...
											<div class="lfr-preview-file-content"
												...
												<div class="lfr-preview-file-image-current-column">
													...
													<span class="lfr-preview-file-actions hide"
				...
				<%
			 		DynamicQuery dynamicQuery = DynamicQueryFactoryUtil.forClass(MBMessage.class, PortletClassLoaderUtil.getClassLoader());
				 	dynamicQuery.add(PropertyFactoryUtil.forName("classPK").eq(fileEntryId));
				 	List<MBMessage> messageList = MBMessageLocalServiceUtil.dynamicQuery(dynamicQuery);
			 		pageContext.setAttribute("commentCount", messageList.size()-1);
				%>		
				
				<%-- <div id="RhizomeCommentBox">
					<c:if test="<%=showAssetMetadata && PropsValues.DL_FILE_ENTRY_COMMENTS_ENABLED%>">
						<liferay-ui:panel collapsible="<%=true%>"
							cssClass="lfr-document-library-comments" extended="<%=true%>"
							persistState="<%=true%>" title="comments">
							<portlet:actionURL var="discussionURL">
								<portlet:param name="struts_action"
									value="/document_library/edit_file_entry_discussion" />
							</portlet:actionURL>

							<liferay-ui:discussion
								className="<%=DLFileEntryConstants.getClassName()%>"
								classPK="<%=fileEntryId%>" formAction="<%=discussionURL%>"
								formName="fm2" ratingsEnabled="<%=enableCommentRatings%>"
								redirect="<%=currentURL%>"
				...
				<%-- <div style="height: 50px !important; float: left;">
					<liferay-ui:ratings
						className="<%=DLFileEntryConstants.getClassName()%>"
						classPK="<%=fileEntryId%>" type="thumbs" />
				</div> --%>

			</div>
		</aui:col>

		<aui:col cssClass="lfr-asset-column-details context-pane"
			last="<%=true%>" width="<%=30%>">
			<div class="body-row asset-details">
				<c:if test="<%=showAssetMetadata%>">
					<div class="asset-details-content">
						<h3
							class="version np-repos-title01 <%=fileEntry.isCheckedOut() ? "document-locked"
								: StringPool.BLANK%>">
							<liferay-ui:message key="version" />
							<%=HtmlUtil.escape(fileVersion.getVersion())%>
						</h3>
						<div class="np-repos-iconfix001">
						<div class="lfr-asset-icon lfr-asset-author">
							<liferay-ui:message
								arguments="<%=HtmlUtil.escape(fileVersion
								.getStatusByUserName())%>"
								key="last-updated-by-x" />
						</div>

						<div class="lfr-asset-icon lfr-asset-date">
							<%=dateFormatDateTime.format(fileVersion
								.getModifiedDate())%>
						</div>

						<c:if test="<%=!portletId.equals(PortletKeys.TRASH)%>">
							<div class="lfr-asset-summary">
								<aui:workflow-status model="<%=DLFileEntry.class%>"
									status="<%=fileVersion.getStatus()%>" />
							</div>
						</c:if>

						<%-- <c:if
							test="<%=Validator.isNotNull(fileVersion
									.getDescription())%>">
							<blockquote class="lfr-asset-description">
								<%=HtmlUtil.escape(fileVersion
									.getDescription())%>
							</blockquote>
						</c:if> --%> 
						<div id="np-repos-download-iconfix">
						<span class="download-document"> <c:if
								test="<%=DLFileEntryPermission.contains(
									permissionChecker, fileEntry,
									ActionKeys.VIEW)%>">
								<liferay-ui:icon image="download" label="<%=true%>"
									...
									+ ")"%>'
									url="<%=DLUtil
									.getPreviewURL(fileEntry, fileVersion,
											themeDisplay, StringPool.BLANK)%>" />
							</c:if>
						</span> </div> <span class="conversions"> <%
 ...
 %> <liferay-ui:icon
						...
						<div class="lfr-asset-field url-file-container hide">
							<aui:field-wrapper name="url">
								<liferay-ui:input-resource id="url"
									url="<%=DLUtil.getPreviewURL(fileEntry,
						...
						<c:if
							test="<%=portletDisplay.isWebDAVEnabled()
									&& fileEntry.isSupportsSocial()%>">
								...
								<%
									String webDavHelpMessage = null;
													...
													if (BrowserSnifferUtil.isWindows(request)) {
														webDavHelpMessage = LanguageUtil
																.format(pageContext,
																		"webdav-windows-help",
																		new Object[]
																		{
														...
														webDavHelpMessage = LanguageUtil
																.format(pageContext, "webdav-help",
																		"http://www.liferay.com/web/guest/community/wiki/-/wiki/Main/WebDAV");
													}
								%>

								<aui:field-wrapper helpMessage="<%=webDavHelpMessage%>"
									name="webdavUrl">
									<liferay-ui:input-resource id="webdavUrl"
										url="<%=webDavUrl%>" />
								</aui:field-wrapper>
							</div>
						</c:if>
				...
				<div class="lfr-asset-panels">
					<liferay-ui:panel-container extended="<%=false%>"
						id="documentLibraryAssetPanelContainer" persistState="<%=true%>">

						<%
							if (fileEntryTypeId > 0) {
											try {
												DLFileEntryType fileEntryType = DLFileEntryTypeServiceUtil
														.getFileEntryType(fileEntryTypeId);

												List<DDMStructure> ddmStructures = fileEntryType
														.getDDMStructures();
												...
												for (DDMStructure ddmStructure : ddmStructures) {
													...
													try {
														...
														fields = StorageEngineUtil
																.getFields(fileEntryMetadata
																		.getDDMStorageId());
													} catch (Exception e) {
													}
						%>

						<liferay-ui:panel collapsible="<%=true%>" cssClass="metadata"
							extended="<%=true%>" id="documentLibraryMetadataPanel"
							persistState="<%=true%>"
							title="<%=HtmlUtil.escape(ddmStructure
												.getName(locale))%>">

							<liferay-ddm:html
								classNameId="<%=PortalUtil
												.getClassNameId(DDMStructure.class)%>"
								classPK="<%=ddmStructure.getPrimaryKey()%>"
								fields="<%=fields%>"
								fieldsNamespace="<%=String.valueOf(ddmStructure
												.getPrimaryKey())%>"
								readOnly="<%=true%>" requestedLocale="<%=locale%>" />
						...
						<liferay-ui:custom-attributes-available
							className="<%=DLFileEntryConstants.getClassName()%>"
							classPK="<%=fileVersionId%>" editable="<%=false%>">
							<liferay-ui:panel collapsible="<%=true%>"
								cssClass="custom-fields"
								id="documentLibraryCustomAttributesPanel"
								persistState="<%=true%>" title="custom-fields">
							...
							try {
											List<DDMStructure> ddmStructures = DDMStructureLocalServiceUtil
													.getClassStructures(
															...
															PortalUtil
																	.getClassNameId(RawMetadataProcessor.class),
															new StructureStructureKeyComparator(
																	true));
											...
											for (DDMStructure ddmStructure : ddmStructures) {
												...
												try {
													DLFileEntryMetadata fileEntryMetadata = DLFileEntryMetadataLocalServiceUtil
															.getFileEntryMetadata(ddmStructure
																	.getStructureId(),
																	fileVersionId);
						...
						<c:if test="<%=showAssetMetadata%>">
							<liferay-ui:panel collapsible="<%=true%>"
								cssClass="version-history"
								id="documentLibraryVersionHistoryPanel"
								persistState="<%=true%>" title="version-history">

								<%
									boolean comparableFileEntry = DocumentConversionUtil
																.isComparableVersion(fileVersion
																		.getExtension());
														...
														PortletURL viewFileEntryURL = renderResponse
																.createRenderURL();

														viewFileEntryURL.setParameter("struts_action",
																"/document_library/view_file_entry");
														viewFileEntryURL.setParameter("redirect",
																currentURL);
														viewFileEntryURL.setParameter("fileEntryId",
																String.valueOf(fileEntry
																		.getFileEntryId()));

														searchContainer
																.setIteratorURL(viewFileEntryURL);
														...
														List results = fileEntry
																.getFileVersions(status);
														List resultRows = searchContainer
																.getResultRows();

														for (int i = 0; i < results.size(); i++) {
															FileVersion curFileVersion = (FileVersion) results
																	.get(i);

															ResultRow row = new ResultRow(new Object[]
															{ fileEntry, curFileVersion,
																	results.size(), conversions,
																	fileEntry.isCheckedOut(),
																	fileEntry.hasLock() },
																	String.valueOf(curFileVersion
																			.getVersion()), i);

															// Statistics

															row.addText(String.valueOf(curFileVersion
																	.getVersion()));
															row.addDate(curFileVersion.getCreateDate());
															row.addText(TextFormatter
																	.formatStorageSize(
																			curFileVersion.getSize(),
																			locale));
															...
															row.addJSP("right",
																	...
																	"/html/portlet/document_library/file_entry_history_action.jsp");
								...
								<portlet:actionURL var="compareVersionsURL">
									<portlet:param name="struts_action"
										value="/document_library/compare_versions" />
								</portlet:actionURL>

								<aui:form action="<%=compareVersionsURL%>" method="post"
									name="fm1"
									...
									<aui:input name="fileEntryId" type="hidden"
										value="<%=fileEntryId%>" />
									<aui:input name="sourceVersion" type="hidden"
										value="<%=curFileVersion.getVersion()%>" />
									<aui:input name="targetVersion" type="hidden"
				...
				<%-- <div id="RhizomeCommentBox">
					...
					<div class="clearfix">
						<div class="np-repos-buttonbox01">
					    	<span>Comments:</span>&nbsp;<span>${commentCount}</span>
					    </div>
					
						<div class="np-repos-buttonbox02">
						    	<!-- <a href="#">Like</a> -->
...
<c:if
	test="<%=DLFileEntryPermission.contains(permissionChecker,
						fileEntry, ActionKeys.VIEW)
						&& DLUtil.isOfficeExtension(fileVersion.getExtension())
						&& portletDisplay.isWebDAVEnabled()
						&& BrowserSnifferUtil.isIeOnWin32(request)%>">
	<%@ include
		file="/html/portlet/document_library/action/open_document_js.jspf"%>
</c:if>
...
<aui:script use="aui-base,aui-toolbar">
	...
	<c:if test="<%=showActions%>">
		...
		<c:if
			test="<%=DLFileEntryPermission
								.contains(permissionChecker, fileEntry,
										ActionKeys.DELETE)
								&& (!(fileEntry.getModel() instanceof DLFileEntry) || !TrashUtil
										.isTrashEnabled(scopeGroupId))%>">
			fileEntryButtonGroup.push(
				{
					<portlet:renderURL var="viewFolderURL">
				<portlet:param name="struts_action" value="/document_library/view" />
				<portlet:param name="folderId"
					value="<%=String.valueOf(fileEntry.getFolderId())%>" />
			</portlet:renderURL>

					icon: 'icon-delete',
					label: '<%=UnicodeLanguageUtil
								.get(pageContext, "delete")%>',
					on: {
						click: function(event) {
...
.sg-header span{ width:auto !important; font-size:26px !important; text-align:left !important;}
...
<%
	if (!portletId.equals(PortletKeys.TRASH)) {
		DLUtil.addPortletBreadcrumbEntries(fileEntry, request,
				renderResponse);
	}
%>