package com.rhizome.url.filter;
...
public class RhizomeURLMappingFilter implements Filter {
	private static final Logger logger = LoggerFactory
			.getLogger(RhizomeURLMappingFilter.class);
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		String currentURL = PortalUtil
				.getCurrentCompleteURL((HttpServletRequest) request);
		
		// 
		// Disable filter mapping for preview content  
		// 
		if (!currentURL.contains("/web/guest/welcome")) {
		...
		if (Validator.isNotNull(friendlyURL)) {
			...
			try {
				...
				RequestDispatcher requestDispatcher = servletContext
						.getRequestDispatcher(URL_PREFIX
								+ group.getFriendlyURL() + redirect);
	...
	private static String createDocumentRedirectURL(long fileEntryId) {
		...
		String redirectURL = DOCUMENT_CONTEXT_PATH + StringPool.QUESTION
				+ PORTLET_ID + StringPool.AMPERSAND + PORTLET_LIFECYCLE
				+ StringPool.AMPERSAND + PORTLET_STATE + StringPool.AMPERSAND
	...
	private static final String DOCUMENT_CONTEXT_PATH = "/repository";
	...
	private static Log _log = LogFactoryUtil
			.getLog(RhizomeURLMappingFilter.class);
}
