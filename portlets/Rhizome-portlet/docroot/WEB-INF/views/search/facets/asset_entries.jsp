...
<div class="search-facet search-asset_entries" data-facetFieldName="entryClassName" id="assetEntryfacet">
	...
	<%
	   ...
	   // keyword = "effects";
		...
		System.out.println("EVERYTHING URL KEYWORD ::: >>> ::: "+keyword);
		...
		<c:if test="${not empty docsMap}">
	        <c:forEach items="${docsMap}" var="doc">
	           <c:set var="astType" value="${doc.key}"/>
	            ...
	            if(!assetType.equals(Solution.class.getName())){
		                ...
		                <c:choose>
		                	...
		                	<c:otherwise>
		                		<a data-value="${doc.key}" href="<%=actionURL.toString()%>" class="seleted-filter">
									...
									<c:choose>
										<c:when test='<%= assetRendererFactory.getTypeName(locale, false).equalsIgnoreCase("Web Content Article") %>'>
											Articles
										</c:when>
										<c:otherwise>
